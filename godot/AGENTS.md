...
- 地图显示位于可裁剪的 `MapClip/MapLayer` 下；`MapLayer` 与 `Residents` 一起缩放，Sidebar 不缩放。滚轮和触控板捏合缩放，范围为 75%–300%；触控板双指平移反转系统手势 delta，使地图内容跟随手指方向，鼠标中/右键拖动保持直接跟手，`0` 恢复视图。点击先将全局坐标转换到 MapLayer 本地坐标，再计算格子。选中居民接近地图视口边缘两格或移出视口时，MapLayer 以指数平滑移动，让居民回到视口中心。
- 选中居民的 `ThoughtBubble` 显示基于财富、当前糖量和视野内资源的本地行动想法；它是表现层提示，不参与居民决策。
